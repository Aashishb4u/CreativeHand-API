...
const deletePortfolioImage = catchAsync(async (req, res) => {
    ...
    const portfolioIndex =  req.body.portfolioIndex;
    if (!business) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Business not found');
    }
    const deleteBusiness = await businessService.deletePortfolioImage(portfolioIndex, req.params.businessId);
    handleSuccess(httpStatus.OK, {deleteBusiness}, 'Portfolio Image deleted Successfully.', req, res);
});
...
const uploadMultiplePortfolioImages = catchAsync(async (req, res) => {
    multipleFileUpload(req, res, (err, data) => {
        if (err || !req.files) {
            handleError(httpStatus.UNSUPPORTED_MEDIA_TYPE, 'Image is not uploaded', req, res, err);
        } else {
            let updateBody = {};
            const fileNames = req.files.map(v => `public/${v.filename}`);
            const oldImages = req.body.oldImages ? JSON.parse(req.body.oldImages) : [];

            businessService.getBusinessPortfolioById(req.params.businessId).then((result) => {
                let existingImages = result.portfolioImages;
                updateBody.portfolioImages = (oldImages &&
                    (oldImages.length < existingImages.length)) ? oldImages.concat(fileNames) :
                    existingImages.concat(fileNames);
                businessService.updateBusinessPortfolioById(req.params.businessId, updateBody).then((result) => {
                    handleSuccess(httpStatus.CREATED, {result}, 'Product Images Uploaded Successfully.', req, res);
                });
            });
